...
class Rating(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('ratings', lazy=True)) # Связь с User
# Форма регистрации
...
class Order(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('orders', lazy=True))  # Связь с User
...
def tovar1():
    ...
    comments = Comment.query.filter_by(product_id=1).order_by(Comment.created_at.desc()).all()  # комментарии для товара
    ratings = Rating.query.filter_by(product_id=1).all()  # оценки для товара
...
def tovar2():
    ...
    comments = Comment.query.filter_by(product_id=2).order_by(Comment.created_at.desc()).all()  # комментарии для товара
    ratings = Rating.query.filter_by(product_id=2).all()  # оценки для товара
...
def tovar3():
    ...
    comments = Comment.query.filter_by(product_id=3).order_by(Comment.created_at.desc()).all()  # комментарии для товара
    ratings = Rating.query.filter_by(product_id=3).all()  #  оценки для товара
...
@login_required
def add_comment():
    comment_text = request.form.get('comment')  #  текст комментария
    product_id = request.form.get('product_id')  # ID
    next_url = request.form.get('next')  # URL для перенаправления
...
@login_required
def add_rating():
    rating_value = request.form.get('rating')  # значение оценки из формы
    product_id = request.form.get('product_id')  # ID товара
    next_url = request.form.get('next')  # URL для перенаправления
...
def checkout():
    ...
    new_order = Order(
        ...
        address=address,
        zip_code=zip_code,
        user_id=current_user.id  # заказ с текущим пользователем
    )
